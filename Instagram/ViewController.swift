...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @objc func handleSignUp() {
        ...
        guard let passwd = passwordTextField.text, passwd.count > 0 else { return }
        
        Auth.auth().createUser(withEmail: email, password: passwd) { (user: User?, error: Error?) in
            ...
            storageItem.putData(uploadData, metadata: nil, completion: { (metadata, err) in
